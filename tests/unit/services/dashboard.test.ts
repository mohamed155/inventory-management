...
describe('dashboard service', () => {
  it('getTotalSalesAmount delegates to electronAPI', async () => {
    const mock = stubElectronAPI({
      getTotalSalesAmount: vi.fn().mockResolvedValue(1500),
    })
  ...
  it('getTotalPurchasesAmount delegates to electronAPI', async () => {
    const mock = stubElectronAPI({
      getTotalPurchasesAmount: vi.fn().mockResolvedValue(800),
    })
  ...
  it('getTotalProfit delegates to electronAPI', async () => {
    const mock = stubElectronAPI({
      getTotalProfit: vi.fn().mockResolvedValue(700),
    })
    const { getTotalProfit } = await import('../../../src/services/dashboard.ts')
  ...
  it('getDueFromCustomers delegates to electronAPI', async () => {
    const mock = stubElectronAPI({
      getDueFromCustomers: vi.fn().mockResolvedValue(200),
    })
  ...
  it('getDueToProviders delegates to electronAPI', async () => {
    const mock = stubElectronAPI({
      getDueToProviders: vi.fn().mockResolvedValue(300),
    })
